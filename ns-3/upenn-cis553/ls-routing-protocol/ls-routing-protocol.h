...
public:
  ...
  struct nghbrTabEntry{
    std::string num_node;
    Ipv4Address nghbr_address;
    Ipv4InterfaceAddress if_address;
  };
  std::vector<struct nghbrTabEntry> NeighborTable;
  LSRoutingProtocol ();
  ...
  void ProcessDscvrNghbr (LSMessage lsMessage);
  void ProcessNghbrRsp (LSMessage lsMessage);  
...
private:
  ...
  // Status
  void AddNeighborTable(std::string num_node, Ipv4Address Ipv4Address, Ipv4InterfaceAddress ifaddress);
  void DumpLSA ();
  void DumpNeighbors ();
